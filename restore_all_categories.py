...
print(f"   Grandsons: {len(grandsons)}")
print(f"   المجموع: {len(parents) + len(children) + len(grandsons)}")

# Create mapping: name -> id
category_map = {}

# Step 1: Create all parent categories (8151-8158)
print(f"\n{'='*80}")
print("Step 1: إنشاء الفئات الرئيسية (Parents)")
print('='*80)

parent_ids = {
    "Grandi elettrodomestici": 8151,
    "Elettrodomestici incasso": 8152,
    "Audio video": 8153,
    "Clima": 8154,
    "Elettrodomestici cucina": 8155,
    "Cura della persona": 8156,
    "Informatica": 8157,
    "Telefonia": 8158
}
...
for parent_name in parents:
    ...
    if not parent_id:
        print(f"⚠️ لم يتم العثور على ID للفئة: {parent_name}")
        continue
    
    # Check if exists
    response = requests.get(f"{BASE_URL}/categories/{parent_id}")
    if response.status_code == 200:
        print(f"✅ موجود: {parent_name} (ID: {parent_id})")
        ...
        data = {
            ...
            "sort_order": list(parent_ids.values()).index(parent_id) + 1,
            "is_active": True,
            "parent_id": None
        }
        ...
        if response.status_code in [200, 201]:
            ...
            cat_id = result.get("data", {}).get("id", parent_id)
            category_map[parent_name] = cat_id
            print(f"✅ تم إنشاء: {parent_name} (ID: {cat_id})")
        else:
            print(f"❌ فشل إنشاء {parent_name}: {response.status_code}")
...
for _, row in df.iterrows():
    ...
    if response.status_code in [200, 201]:
        ...
        created_children += 1
        print(f"✅ {child_name} (ID: {cat_id}) -> تحت {parent_name}")
    else:
        print(f"❌ فشل إنشاء {child_name}: {response.status_code}")

print(f"\n✅ تم إنشاء {created_children} طفل")
...
for _, row in df.iterrows():
    ...
    if response.status_code in [200, 201]:
        ...
        created_grandsons += 1
        print(f"✅ {grandson_name} (ID: {cat_id}) -> تحت {child_name}")
    else:
        print(f"❌ فشل إنشاء {grandson_name}: {response.status_code}")

print(f"\n✅ تم إنشاء {created_grandsons} حفيد")
